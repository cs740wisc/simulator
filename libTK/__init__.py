...
class NetworkOut(TKSenderReconnect):
    """
        Uses TKSenderReconnect to generate a client TCP connection to a message
        ...
        This class is expected to be called in the following way:
            ...
            # During init code:
            # Now use your reference to TK to change the "out" class
            TK.out.info = TK.Stdout(TK.Colors.INFO, [TK.NetworkOut(None, '/tmp/log')])
            ...
            TK.out.info.other_out[0].run("MemorableID like my GUID", settings.MSG_SERVER_ADDR, settings.MSG_SERVER_PORT)
    ...
    def run(self, myid, addr, port):
        ...
        self.addr = addr
        self.port = port
        TKSenderReconnect.__init__(self, None, addr, port)
    
    def connectionLost(self, proto, reason):
        """@overwrite
            This function overwrites the default stub function found in TKSender."""
        self.proto = None
